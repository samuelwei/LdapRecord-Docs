...
- Do not prepend your protocol (`ldap://` or `ldaps://`) to your hosts. <br/> Use the `use_tls` configuration option instead.
</Important>
...
<Important>
- If enabling TLS (`use_tls`), and `port` is set to `389`, it will be automatically overridden to use `636`.
- If enabling STARTTLS (`use_starttls`), you must use the default port for your LDAP server (`389`). TLS ports cannot be used.
</Important>

### TLS & STARTTLS

These boolean options enable a secure connection to your LDAP server:
...
- `use_tls` - Connects using the `ldaps://` protocol on port 636 by default
- `use_starttls` - Connects using the `ldap://` protocol on port 389, then upgrades to a secure connection using STARTTLS
...
- You **must** enable TLS or STARTTLS to set / change / reset passwords in Active Directory.
- **STARTTLS is recommended over TLS**. It allows you to use the standard LDAP port while still encrypting your connection.
</Important>
...
<Important>
If the timeout is reached performing an LDAP operation and
...
<Important>
It's heavily recommended to use version three (`3`). You may experience issues using version two (`2`).
</Important>
